...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply>
    ...
    public List<SeckillGoodsVO> getSeckillGoods(Integer timeline) {
        try {
            ...
            Optional<SeckillTimelineVO> first = seckillTimelineToCache.stream()
                    .filter(i -> i.getTimeLine().equals(timeline)).findFirst();
            if (first.isPresent()) {
    ...
    public IPage<SeckillApply> getSeckillApplyPage(SeckillSearchParams queryParam, PageVO pageVo) {
        ...
        if (seckillApplyPage != null && !seckillApplyPage.getRecords().isEmpty()) {
            ...
            if (!skuIds.isEmpty()) {
                List<Integer> skuStock = promotionGoodsService.getPromotionGoodsStock(PromotionTypeEnum.SECKILL,
                        queryParam.getSeckillId(), skuIds);
    ...
    public void addSeckillApply(String seckillId, String storeId, List<SeckillApplyVO> seckillApplyList) {
        ...
        for (SeckillApplyVO seckillApply : seckillApplyList) {
            ...
            // 获取秒杀活动时间段
            DateTime startTime = DateUtil.offsetHour(DateUtil.beginOfDay(seckill.getStartTime()),
        ...
        boolean result = true;
        this.remove(new LambdaQueryWrapper<SeckillApply>().eq(SeckillApply::getSeckillId, seckillId)
        ...
        if (!promotionGoodsList.isEmpty()) {
            ...
            searchParams
                    .setSkuIds(promotionGoodsList.stream().map(PromotionGoods::getSkuId).collect(Collectors.toList()));
    ...
    public boolean updateSeckillApplyTime(Seckill seckill) {
        ...
        List<SeckillApply> list = this.list(queryWrapper).stream()
                .filter(i -> i.getTimeLine() != null && seckill.getHours().contains(i.getTimeLine().toString()))
        ...
        if (!promotionGoodsList.isEmpty()) {
            ...
            promotionGoodsService.deletePromotionGoods(searchParams);
            // 初始化促销商品
            List<PromotionGoods> promotionGoods = PromotionTools.promotionGoodsInit(promotionGoodsList, seckill,
            ...
            deleteWrapper.notIn(SeckillApply::getSkuId,
    ...
    private List<SeckillTimelineVO> getSeckillTimelineInfo() {
        ...
        for (Seckill seckill : seckillList) {
            ...
            for (int i = 0; i < hoursSored.length; i++) {
                ...
                boolean hoursSoredHour = (hoursSored[i] >= hour
                        || ((i + 1) < hoursSored.length && hoursSored[i + 1] > hour));
                ...
                if (hoursSoredHour || lastHour) {
                    ...
                    // 秒杀活动的时刻
                    long timeLine = cn.lili.common.utils.DateUtil.getDateline(date + " " + hoursSored[i],
                            "yyyy-MM-dd HH");
    ...
    private List<SeckillGoodsVO> wrapperSeckillGoods(Integer startTimeline, String seckillId) {
        ...
        List<SeckillApply> seckillApplyList = this
                .list(new LambdaQueryWrapper<SeckillApply>().eq(SeckillApply::getSeckillId, seckillId));
     ...
     */
    private void checkSeckillGoodsSku(Seckill seckill, SeckillApplyVO seckillApply, GoodsSku goodsSku,
            DateTime startTime) {
        // 活动库存不能大于商品库存
        ...
        // 查询是否在同一时间段参与了拼团活动
        if (promotionGoodsService.findInnerOverlapPromotionGoods(PromotionTypeEnum.PINTUAN.name(), goodsSku.getId(),
                startTime, seckill.getEndTime(), seckill.getId()) > 0) {
            throw new ServiceException("商品[" + goodsSku.getGoodsName() + "]已经在重叠的时间段参加了拼团活动，不能参加秒杀活动");
        }
        // 查询是否在同一时间段参与了秒杀活动活动
        if (promotionGoodsService.findInnerOverlapPromotionGoods(PromotionTypeEnum.SECKILL.name(), goodsSku.getId(),
                startTime, seckill.getEndTime(), seckill.getId()) > 0) {
