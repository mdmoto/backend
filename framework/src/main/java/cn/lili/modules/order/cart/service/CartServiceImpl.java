...
public class CartServiceImpl implements CartService {
    ...
    public void add(String skuId, Integer num, String cartType, Boolean cover) {
        ...
        try {
            ...
            if (cartTypeEnum.equals(CartTypeEnum.CART)) {
                ...
                CartSkuVO cartSkuVO = cartSkuVOS.stream().filter(i -> i.getGoodsSku().getId().equals(skuId)).findFirst()
                        .orElse(null);
    ...
    public Long getCanUseCoupon(CartTypeEnum checkedWay) {
        ...
        if (tradeDTO.getSkuList() != null && !tradeDTO.getSkuList().isEmpty()) {
            List<String> ids = tradeDTO.getSkuList().stream().filter(i -> Boolean.TRUE.equals(i.getChecked()))
                    .map(i -> i.getGoodsSku().getId()).collect(Collectors.toList());
            ...
            for (EsGoodsIndex esGoodsIndex : esGoodsList) {
                if (esGoodsIndex != null && esGoodsIndex.getPromotionMap() != null
                        && !esGoodsIndex.getPromotionMap().isEmpty()) {
                    List<String> couponIds = esGoodsIndex.getPromotionMap().keySet().stream()
                            .filter(i -> i.contains(PromotionTypeEnum.COUPON.name()))
                            .map(i -> i.substring(i.lastIndexOf("-") + 1)).collect(Collectors.toList());
                    if (!couponIds.isEmpty()) {
                        List<MemberCoupon> currentGoodsCanUse = memberCouponService
                                .getCurrentGoodsCanUse(tradeDTO.getMemberId(), couponIds, totalPrice);
            ...
            // 获取可操作的优惠券集合
            List<MemberCoupon> allScopeMemberCoupon = memberCouponService
                    .getAllScopeMemberCoupon(tradeDTO.getMemberId(), storeIds);
    ...
    private void checkSetGoodsQuantity(CartSkuVO cartSkuVO, String skuId, Integer num) {
        ...
        if (cartSkuVO.getGoodsSku() != null
                && !GoodsSalesModeEnum.WHOLESALE.name().equals(cartSkuVO.getGoodsSku().getSalesModel())
                && cartSkuVO.getNum() > 99) {
            cartSkuVO.setNum(99);
        }
    }
    ...
    public Long getCartNum(Boolean checked) {
        ...
        // 过滤sku列表
        List<CartSkuVO> collect = tradeDTO.getSkuList().stream().filter(i -> Boolean.FALSE.equals(i.getInvalid()))
    ...
    public Trade createTrade(TradeParams tradeParams) {
        ...
        // 订单无收货地址校验
        if (tradeDTO.getStoreAddress() == null && tradeDTO.getMemberAddress() == null && !GoodsTypeEnum.VIRTUAL_GOODS
                .name().equals(tradeDTO.getCheckedSkuList().get(0).getGoodsSku().getGoodsType())) {
    ...
    private void useCoupon(TradeDTO tradeDTO, MemberCoupon memberCoupon, CartTypeEnum cartTypeEnum) {
        ...
        for (CartSkuVO cartSkuVO : cartSkuVOS) {
            ...
            if (cartSkuVO.getPromotionMap() != null && !cartSkuVO.getPromotionMap().isEmpty()) {
                if (cartSkuVO.getPromotionMap().keySet().stream()
                        .anyMatch(i -> i.contains(PromotionTypeEnum.PINTUAN.name())
                                || i.contains(PromotionTypeEnum.SECKILL.name()))) {
    ...
    private List<CartSkuVO> checkCoupon(MemberCoupon memberCoupon, TradeDTO tradeDTO) {
        ...
        if (Boolean.FALSE.equals(memberCoupon.getPlatformFlag())) {
            cartSkuVOS = tradeDTO.getSkuList().stream().filter(i -> i.getStoreId().equals(memberCoupon.getStoreId()))
                    .collect(Collectors.toList());
        }
        // 否则为平台优惠券，筛选商品为全部商品
        else {
        ...
        if (memberCoupon.getScopeType().equals(PromotionsScopeTypeEnum.ALL.name())) {
            ...
            return cartSkuVOS.stream().filter(
                    i -> CharSequenceUtil.contains(memberCoupon.getScopeId(), i.getGoodsSku().getCategoryPath()))
                    .collect(Collectors.toList());
        } else if (memberCoupon.getScopeType().equals(PromotionsScopeTypeEnum.PORTION_GOODS.name())) {
            // 范围关联ID是否包含
            return cartSkuVOS.stream()
                    .filter(i -> CharSequenceUtil.contains(memberCoupon.getScopeId(), i.getGoodsSku().getId()))
                    .collect(Collectors.toList());
        } else if (memberCoupon.getScopeType().equals(PromotionsScopeTypeEnum.PORTION_SHOP_CATEGORY.name())) {
            // 店铺分类路径是否包含
            return cartSkuVOS.stream().filter(
                    i -> CharSequenceUtil.contains(memberCoupon.getScopeId(), i.getGoodsSku().getStoreCategoryPath()))
                    .collect(Collectors.toList());
        }
    ...
    private void checkGoodsSaleModel(GoodsSku dataSku, List<CartSkuVO> cartSkuVOS) {
        if (dataSku.getSalesModel().equals(GoodsSalesModeEnum.WHOLESALE.name())) {
            int numSum = 0;
            List<CartSkuVO> sameGoodsIdSkuList = cartSkuVOS.stream()
                    .filter(i -> i.getGoodsSku().getGoodsId().equals(dataSku.getGoodsId()))
                    .collect(Collectors.toList());
    ...
    private void checkPintuan(CartSkuVO cartSkuVO) {
        ...
        // 获取拼团信息
        if (cartSkuVO.getPromotionMap() != null && !cartSkuVO.getPromotionMap().isEmpty()) {
            Optional<Map.Entry<String, Object>> pintuanPromotions = cartSkuVO.getPromotionMap().entrySet().stream()
                    .filter(i -> i.getKey().contains(PromotionTypeEnum.PINTUAN.name())).findFirst();
    ...
    private void checkKanjia(CartSkuVO cartSkuVO) {
        if (cartSkuVO.getPromotionMap() != null && !cartSkuVO.getPromotionMap().isEmpty()) {
            Optional<Map.Entry<String, Object>> kanjiaPromotions = cartSkuVO.getPromotionMap().entrySet().stream()
                    .filter(i -> i.getKey().contains(PromotionTypeEnum.KANJIA.name())).findFirst();
