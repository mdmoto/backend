...
public class TradeServiceImpl extends ServiceImpl<TradeMapper, Trade> implements TradeService {
    ...
    public Trade createTrade(TradeDTO tradeDTO) {
        ...
        applicationEventPublisher
                .publishEvent(new TransactionCommitSendMQEvent("订单创建消息", rocketmqCustomProperties.getOrderTopic(),
    ...
    private void createTradeCheck(TradeDTO tradeDTO) {
        ...
        if (!tradeDTO.getCartTypeEnum().equals(CartTypeEnum.VIRTUAL) && tradeDTO.getStoreAddress() == null) {
            ...
            if (memberAddress == null && !GoodsTypeEnum.VIRTUAL_GOODS.name()
            ...
            if (tradeDTO.getCartList().stream()
                    .allMatch(item -> item.getDeliveryMethod().equals(DeliveryMethodEnum.SELF_PICK_UP.name()))) {
    ...
    private void pointPretreatment(TradeDTO tradeDTO) {
        ...
        if (tradeDTO.getPriceDetailDTO() != null
            ...
            boolean result = memberService.updateMemberPoint(tradeDTO.getPriceDetailDTO().getPayPoint(),
                    ...
                    "订单【" + orderSns + "】创建，喵币扣减", "DEDUCT_TRADE_" + tradeDTO.getSn(), java.math.BigDecimal.ZERO);
