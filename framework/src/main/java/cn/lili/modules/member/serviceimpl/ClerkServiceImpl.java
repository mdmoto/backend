...
public class ClerkServiceImpl extends ServiceImpl<ClerkMapper, Clerk> implements ClerkService {
    ...
    public IPage<ClerkVO> clerkForPage(PageVO page, ClerkQueryDTO clerkQueryDTO) {
        ...
        clerkVOQueryWrapper.eq(StringUtils.isNotEmpty(clerkQueryDTO.getDepartmentId()), "li_clerk.department_id",
                clerkQueryDTO.getDepartmentId());
        clerkVOQueryWrapper.like(StringUtils.isNotEmpty(clerkQueryDTO.getClerkName()), "li_clerk.clerk_name",
                clerkQueryDTO.getClerkName());
        clerkVOQueryWrapper.like(StringUtils.isNotEmpty(clerkQueryDTO.getMobile()), "m.mobile",
                clerkQueryDTO.getMobile());
         ...
         * 
         * List<StoreDepartment> departments = storeDepartmentService.list(new
         * QueryWrapper<StoreDepartment>()
         * .eq("store_id", UserContext.getCurrentUser().getStoreId()));
         * 
         ...
         * });
         * List<Member> members = memberService.list(new QueryWrapper<Member>().in("id",
         * memberIds));
         * 
         ...
         * result.add(clerkVO);
         * });
         * Page<ClerkVO> pageResult = new Page(clerkPage.getCurrent(),
         * clerkPage.getSize(), clerkPage.getTotal());
        ...
        if (clerk != null) {
            ...
            if (Boolean.TRUE.equals(clerkEditDTO.getIsSuper())) {
                ...
                if (!clerkEditDTO.getRoles().isEmpty()) {
                    ...
                    clerkEditDTO.getRoles().forEach(a -> storeClerkRoleList
                            .add(StoreClerkRole.builder().clerkId(clerk.getId()).roleId(a).build()));
    ...
    public Clerk saveClerk(ClerkAddDTO clerkAddDTO) {
        ...
        // 部门校验
        if (CharSequenceUtil.isNotEmpty(clerkAddDTO.getDepartmentId())
                && storeDepartmentService.getById(clerkAddDTO.getDepartmentId()) == null) {
            throw new ServiceException(ResultCode.PERMISSION_NOT_FOUND_ERROR);
        }
        // 角色校验
        ...
        if (Boolean.FALSE.equals(clerkAddDTO.getIsSuper()) && clerkAddDTO.getRoles() != null) {
            ...
            clerkAddDTO.getRoles().forEach(
                    a -> storeClerkRoleList.add(StoreClerkRole.builder().clerkId(clerk.getId()).roleId(a).build()));
