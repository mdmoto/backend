...
public class SettingManagerController {
    ...
    @ApiImplicitParam(name = "key", value = "配置key", paramType = "path", allowableValues = "BASE_SETTING,EMAIL_SETTING,GOODS_SETTING,LOGISTICS_SETTING,ORDER_SETTING,OSS_SETTING,POINT_SETTING,"
            +
    ...
    @ApiImplicitParam(name = "key", value = "配置key", paramType = "path", allowableValues = "BASE_SETTING,EMAIL_SETTING,GOODS_SETTING,KUAIDI_SETTING,ORDER_SETTING,OSS_SETTING,POINT_SETTING,"
            +
            "WECHAT_PC_CONNECT,WECHAT_WAP_CONNECT,WECHAT_APP_CONNECT,WECHAT_MP_CONNECT," +
            "QQ_WEB_CONNECT,QQ_APP_CONNECT," +
            "QQ_WEB_CONNECT,QQ_APP_CONNECT,WEIBO_CONNECT,ALIPAY_CONNECT," +
    ...
    private String filter(SettingEnum settingEnum, String configValue) {
        if (settingEnum.equals(SettingEnum.POINT_SETTING)) {
            ...
            // 强制将注册、签到、评价积分设为0，防止管理员修改
            pointSetting.setRegister(0);
            pointSetting.setSignIn(0);
            pointSetting.setComment(0);
            // 同时清空连续签到的阶梯奖励设置，彻底禁用签到积分
            pointSetting.setPointSettingItems(new ArrayList<>());
